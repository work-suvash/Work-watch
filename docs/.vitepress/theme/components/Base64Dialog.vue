...
const openLink = () => {
  if (dontShowAgain.value) {
    localStorage.setItem('workwatch-base64-dialog-preference', 'true')
  }
    ...
    <div v-show="show" class="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click="close">
      ...
      >
        ...
        <ul class="list-disc list-inside mb-4 space-y-2 text-text-1">
          ...
          <li>
            A userscript: <a href="https://greasyfork.org/en/scripts/485772-workwatch-base64-auto-decoder" target="_blank" rel="noreferrer" class="text-primary hover:underline font-medium">WORKWATCH Base64 Auto Decoder</a> (using a <a href="/internet-tools#userscripts" target="_blank" class="text-primary hover:underline font-medium">userscript manager</a>)
          </li>
        </ul>
