...
if (safeEnv('WORKWATCH_BUILD_NSFW') === 'false') {
    meta.build.nsfw = false
}
if (safeEnv('WORKWATCH_BUILD_API') === 'false') {
    meta.build.api = false
}

const formatCommitRef = (commitRef: string) =>
    `<a href="https://github.com/workwatch/edit/commit/${commitRef}">${commitRef.slice(0, 8)}</a>`
...
// Social media icons intentionally removed from the navbar for a cleaner UX.
